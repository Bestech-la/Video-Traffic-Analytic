...
def detect_and_crop_car_plate(image_path, output_path, color_lower, color_upper, color_name, min_width=900, min_height=300):
    try:
        ...
        resized_plate_path_color_mask = f'captured_images/color_mask_{color_name.lower()}_car_plate_{str(len(os.listdir("captured_images")) + 1)}.png'
        cv2.imwrite(resized_plate_path_color_mask, color_mask)
        
        cv2.imwrite(resized_plate_path, resized_plate_path_contours)
        contours = sorted(contours, key=cv2.contourArea, reverse=True)[:1]
        
        # Save the image with contours (assuming this is your intention)
        resized_plate_path_contours = f'captured_images/resized_plate_path_contours_{color_name.lower()}_car_plate_{str(len(os.listdir("captured_images")) + 1)}.png'
        cv2.drawContours(img, contours, -1, (0, 255, 0), 3) 
        cv2.imwrite(resized_plate_path_contours, img)
