                    ...
                    if y + h > red_line_y:
                        ...
                        if car_id not in captured_car_plates:
                            ...
                            red_mask = cv2.inRange(
                                hsv_car_plate, lower_red, upper_red)
                            contours_red, _ = cv2.findContours(
                                red_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

                            if contours_red:
                                max_red_area = max(
                                    contours_red, key=cv2.contourArea)
                                x_red, y_red, w_red, h_red = cv2.boundingRect(
                                    max_red_area)

                                red_car_plate = hsv_roi[y_red:y_red +
                                                        h_red, x_red:x_red + w_red]

                                if red_car_plate.shape[1] < min_width or red_car_plate.shape[0] < min_height:
                                    aspect_ratio_red = red_car_plate.shape[1] / \
                                        red_car_plate.shape[0]
                                    new_width_red = min(min_width, int(
                                        min_height * aspect_ratio_red))
                                    new_height_red = min(min_height, int(
                                        min_width / aspect_ratio_red))
                                    ...
                                    cv2.imwrite(
                                        img_path_red, red_car_plate_resized)
                                    gray_region = cv2.cvtColor(
                                        red_car_plate_resized, cv2.COLOR_BGR2GRAY)

                                    custom_config = r'--oem 3 --psm 6 -l Lao.carPlate+lao'
            ...
            for index in matched_indices:
                ...
                if len(text_part) == 2 and number_part.isdigit() and len(number_part) == 4:
                    ...
                    if car_plate not in captured_car_plates:
                        ...
                        img_path_png = os.path.splitext(
                            matched_plate_image_path)[0] + ".png"
                        Image.open(matched_plate_image_path).save(img_path_png)

                        with open(img_path_png, 'rb') as matched_plate_image_file:
                            matched_plate_image = File(
                                matched_plate_image_file)
                            ...
                            # Save the PNG image file to the appropriate field
                            report.image_one.save(os.path.basename(
                                img_path_png), matched_plate_image)

                            matched_plate_image_file.close()

                        # Remove the original image file
                        os.remove(matched_plate_image_path)
